...
* `src/opsc` The source code of the _Oopsie_ checker (mainline Java 17+ edition)
* `src/opsc-java8` The source code of the _Oopsie_ checker (Java 8 edition for OSCAR)
* `projects` The example projects used for the experiments
* `scripts` Scripts run during the experiments
* `data/generated` The artifacts generated by the experiments
* `data/expected` The expected artifacts of the experiments for comparison
* `logs` Output generated by the experiments (e.g., compilation logs, checker output, etc.)
* `start_main.sh` and `start_performance.sh`: Scripts to start the experiments (see below)
